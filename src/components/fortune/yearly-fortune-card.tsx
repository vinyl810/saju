...
import type { YearlyFortune } from '@/lib/saju/types';
...
export function YearlyFortuneCard({ fortune }: YearlyFortuneCardProps) {
  const [expanded, setExpanded] = useState(false);

  return (
    <Card
      className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent cursor-pointer transition-shadow hover:shadow-md"
      onClick={() => setExpanded(!expanded)}
    >
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-sm">
          <span className="font-serif text-lg text-primary">{fortune.year}</span>
          <TermTooltip termKey="세운">세운 (歲運)</TermTooltip>
